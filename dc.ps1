if ((Get-WindowsFeature AD-Domain-Services | select-object -ExpandProperty "InstallState") -ne "Installed"){
 Add-WindowsFeature AD-Domain-Services -IncludeManagementTools # install ADDS
}
if ((hostname) -ne "DC-1"){
...
try {
	...
	if ((!(type C:\Windows\Temp\ad.txt)) -eq "Done"){
		echo "Done" >> C:\Windows\Temp\ad.txt
